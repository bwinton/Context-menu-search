...
exports.main = () => {
    ...
    function myListener() {
        ...
        if (!selAnchorNode) {
            ...
            clientYDoc = currentElBox.top + currentElBox.height + 6;
        }
        else {
            if (sel.rangeCount) {
...
// Add New panel
    ...
    function maker(x, y) {
        ...
        if (searchResultPanel == null) {
            ...
            //searchResultPanel.port.on('hideIconT', () => {
            //    let currentBro = browserDocument.getElementById('content').selectedBrowser;
    ...
    function hideIcon() {
        var miniD = browserDocument.getElementById('minimizerDots');//.selectedBrowser //browserDocument.getAnonymousElementByAttribute(browserDocument.querySelector('#content'), 'id', 'minimizerDots');
        if (miniD) {
    ...
    //browserWindow.addEventListener('click', hideIcon);
};
